...
export default function Header() {
  ...
  const ref = "nav"; // Giả sử ref được gán như thế này.
    ...
    <div className="h-auto flex flex-col items-center bg-background pt-20">
      <header className="w-full bg-card p-4 flex justify-between items-center border-b fixed top-0 left-0 right-0 z-50">
        <div className="flex items-center space-x-4">
          ...
          <Link href="/drops">
            <span className="cursor-pointer hover:text-primary hover:bg-slate-300 rounded-lg px-2 py-1">
              Drops
            </span>
          </Link>
          <Link href="/stats">
          ...
          {/* Sửa phần Create */}
          <Link href={`/studio/create?ref=${ref}`}>
            <span className="cursor-pointer hover:text-primary hover:bg-slate-300 rounded-lg px-2 py-1">
              Create
            </span>
          </Link>
        ...
        <div className="flex justify-center flex-1 mx-4">
          <div className="flex items-center space-x-2 border rounded-lg px-3 py-2">
            ...
            <input
              ...
              className="bg-transparent border-none outline-none min-w-[500px] h-[25px] text-sm placeholder:text-muted-foreground"
            />
          </div>
        </div>
